  ...
  const todosList = todos.map((todo) => (
    <tr key={todo.id}>
      ...
      <td className="text-center">
        {editTodoId === todo.id ? (
          <button className="btn btn-edit btn-sm mr-2" onClick={() => handleSave(todo.id)}>
            Save
          </button>
        ) : (
          <button className="btn btn-edit btn-sm mr-2" onClick={() => handleEdit(todo)}>
            Edit
          </button>
        )}
        <button className="btn btn-delete btn-sm" onClick={() => handleDelete(todo.id)}>
          Delete
        </button>
      </td>
    </tr>
